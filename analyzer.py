...
def evaluate_move(score_before: float, score_after: float, color: str) -> str:
    """
    Analyse l'impact d'un coup sur la position et retourne une annotation style Chess.com.
    - score_* en centipions (0.00 = égalité, >0.0 avantage blanc, <0.0 avantage noir)
    """

    delta = score_after - score_before if color == "white" else score_before - score_after

    # Classification basée sur delta (simplifiée)
    if delta >= 150:
        return "Brillant"
    elif delta >= 80:
...
async def analyze_fen_sequence(fen_before: str, fen_after: str, color: str) -> Optional[Tuple[str, str]]:
    """
    Analyse le passage d'une position FEN à une autre, en renvoyant :
    - Le score centipion du moteur avant et après
    - Une annotation descriptive
    """
