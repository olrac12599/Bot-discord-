...
async def analyze_fen_sequence(fen_before: str, fen_after: str, color: str):
    before = await query_lichess_analysis(fen_before)
    after = await query_lichess_analysis(fen_after)
    if not before or not after:
        return None
    try:
        score_before = before["pvs"][0]["cp"]
        score_after = after["pvs"][0]["cp"]
